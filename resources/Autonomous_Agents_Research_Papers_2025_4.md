...
[MobileWorld: Benchmarking Autonomous Mobile Agents in Agent-User Interactive and MCP-Augmented Environments](http://arxiv.org/abs/2512.19432)

- MobileWorld: introduces a substantially more challenging mobile-use online benchmark designed to reflect real-world usage, featuring the Host Machine, GUI Agent, LLM-Simulated User Agent, Planner, Grounding Executor, MCP Servers, Docker Environment, Eval Controller, and Hybrid Action Space.
- The benchmark emphasizes long-horizon, cross-application workflows and introduces novel task categories: agent-user interaction (22.4%) and Model Context Protocol (MCP)-augmented tasks (19.9%), requiring hybrid GUI and external tool usage.
- MobileWorld utilizes a fully observable and controlled Docker environment with self-hosted open-source applications and multiple verification methods to enable rigorous, deterministic evaluation, addressing saturation issues found in prior benchmarks like AndroidWorld.
...
[Mechanism-Based Intelligence (MBI): Differentiable Incentives for Rational Coordination and Guaranteed Alignment in Multi-Agent Systems](http://arxiv.org/abs/2512.20688)

- MBI (Mechanism-Based Intelligence): introduces a paradigm shift for multi-agent systems, replacing centralized cognitive models (like LLMs) with decentralized coordination enforced by the Differentiable Price Mechanism (DPM).
- The MBI architecture uses a Planner to define the Differentiable Directed Acyclic Graph (D-DAG) and the Global Loss, ensuring agents (modeled with Bounded Rationality) maximize local utility aligned with the global optimum.
- The DPM computes the Incentive Signal ($G_i$) as the negative marginal gradient of the global loss, providing VCG-equivalent feedback that guarantees Dominant Strategy Incentive Compatibility (DSIC) and linear scaling $O(N)$.
...
[Does It Tie Out? Towards Autonomous Legal Agents in Venture Capital](http://arxiv.org/abs/2512.18658)

- EQUALL (Eager Construction of a World Model): introduces an architecture for autonomous legal agents performing capitalization tie-out, featuring Foundational Extraction (Stage 1 processing) and Inductive Event Modeling (Stage 2 organization) to build an Event Graph (Structured legal history), followed by Neuro-Symbolic Verification (Targeted query execution).
- The system overcomes limitations of lazy RAG approaches by decoupling extraction from verification, creating a robust, layered world model from unstructured legal documents (Dataroom (D)) to infer the Virtual Cap Table ($C_{virt}$).
- Verification is reduced to deterministic queries over the Event Graph, enabling reliable anomaly detection (Missing Documentation, Inconsistent Terms) and achieving high F1 scores across scaling complexity where agentic baselines fail.

---

#### 19th December 2025

[ReX-MLE: The Autonomous Agent Benchmark for Medical Imaging Challenges](http://arxiv.org/abs/2512.17838)

- ReX-MLE (Autonomous Agent Benchmark): introduces a comprehensive benchmark of 20 medical imaging challenges to evaluate autonomous LLM-based agents' ability to handle full end-to-end scientific workflows, including data preprocessing, model training, and submission.
- The benchmark requires agents to operate within a Medical Image Challenge Environment, iteratively performing strategy generation, error analysis, coding, debugging, and model training to produce submission-ready prediction files.
- Evaluation of state-of-the-art agents (AIDE, ML-Master, R&D-Agent) using powerful LLM backends (GPT-5, Gemini, Claude) reveals a severe performance gap, with most submissions ranking in the 0th percentile compared to human experts.

---

[Binding Agent ID: Unleashing the Power of AI Agents with accountability and credibility](http://arxiv.org/abs/2512.17538)

- BAID (Binding Agent ID): introduces a comprehensive identity infrastructure for autonomous AI agents, integrating local biometric binding, decentralized on-chain identity management, and a zkVM-based authentication protocol.
- The framework establishes verifiable user-code binding by leveraging recursive proofs to treat the program binary as the identity, ensuring operator identity, configuration integrity, and complete execution provenance.
- BAID addresses critical security requirements like user identity verification, trusted discovery, mutual authentication, and accountability tracking through cryptographic guarantees suitable for decentralized agent ecosystems.
...
[From Code Foundation Models to Agents and Applications: A Comprehensive Survey and Practical Guide to Code Intelligence](http://arxiv.org/abs/2511.18538)

- The Code LLM Ecosystem Survey: introduces a comprehensive synthesis and practical guide covering the complete model life cycle from data curation, Pre-training, Supervised Fine-tuning (SFT), and Reinforcement Learning (RL) to Code Agent development.
- The survey evaluates Code LLMs across diverse architectures, including Encoder-only, Encoder-Decoder, Decoder-only (Dense/MoE), and Diffusion-based models, tracing their evolution toward specialized code intelligence.
- It systematically examines advanced concepts like autonomous Code Agents, hierarchical architectures (Knowledge Graph/Semantic Layer), and critical safety dimensions, including verifiable rewards and adversarial robustness.
...
[FronTalk: Benchmarking Front-End Development as Conversational Code Generation with Multi-Modal Feedback](http://arxiv.org/abs/2601.04203)

- FronTalk (Conversational Code Generation Benchmark): introduces a realistic benchmark for multi-turn front-end coding, integrating conversational dynamics and multi-modal feedback (textual and visual instructions) via User Simulators and evaluated by an Agent-Based Evaluation Framework.
- The Agent-Based Evaluation Framework uses an interactive Web Agent equipped with Image Manipulation Tools to measure functional correctness (Pass Rate) and user experience (Usability) of the Generated Website.
- The paper proposes ACECODER, an Agent-Based Critique Method, which leverages the Web Agent to verify the implementation of all past and current instructions, generating feedback to mitigate the significant forgetting issue observed in multi-turn interactions.
...
[Autonomous QA Agent: A Retrieval-Augmented Framework for Reliable Selenium Script Generation](http://arxiv.org/abs/2601.06034)

- Autonomous QA Agent: introduces a Retrieval-Augmented Generation (RAG) framework for reliable Selenium script generation, utilizing a multi-modal ingestion pipeline to index textual requirements and structural HTML/DOM context.
- The system is architected as a five-layer, microservices-based architecture, employing ChromaDB as the vector store and Llama 3.1-8b-instant LLM for context-aware code generation.
- By grounding script generation in actual DOM structure, the RAG approach achieves 90% execution success, significantly mitigating the UI selector hallucination common in standard LLM generation (30% success).
